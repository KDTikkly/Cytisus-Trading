...
public partial class App : Application
{
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        if (e.Args.Length > 0 &&
            string.Equals(e.Args[0], "--prompt5-smoke", StringComparison.Ordinal))
        {
            var rootDirectory = e.Args.Length > 1
                ? e.Args[1]
                : Path.Combine(
                    Path.GetTempPath(),
                    $"cytisus-prompt5-{Guid.NewGuid():N}");
            ShutdownMode = ShutdownMode.OnExplicitShutdown;
            Shutdown(Prompt5Smoke.Run(rootDirectory));
            return;
        }
                ...
                StringComparison.Ordinal))
        {
            var rootDirectory = e.Args.Length > 1
                ...
                : Path.Combine(
                    Path.GetTempPath(),
                    $"cytisus-model-providers-{Guid.NewGuid():N}");
            ShutdownMode = ShutdownMode.OnExplicitShutdown;
            Shutdown(ModelProviderSmoke.Run(rootDirectory));
            return;
        }

        if (e.Args.Length > 0 &&
            string.Equals(e.Args[0], "--v112-smoke", StringComparison.Ordinal))
        {
            var rootDirectory = e.Args.Length > 1
                ? e.Args[1]
                : Path.Combine(
                    Path.GetTempPath(),
                    $"cytisus-v112-{Guid.NewGuid():N}");
            ShutdownMode = ShutdownMode.OnExplicitShutdown;
            Shutdown(V112Smoke.Run(rootDirectory));
            return;
        }

        if (e.Args.Length > 0 &&
            string.Equals(e.Args[0], "--v113-smoke", StringComparison.Ordinal))
        {
            var rootDirectory = e.Args.Length > 1
                ? e.Args[1]
                : Path.Combine(
                    Path.GetTempPath(),
                    $"cytisus-v113-{Guid.NewGuid():N}");
            ShutdownMode = ShutdownMode.OnExplicitShutdown;
            Shutdown(V113Smoke.Run(rootDirectory));
            return;
        }
